...
static void set_own_loc(afl_state_t *afl, char *argv0) {
...
void save_exec_cmdline(afl_state_t *afl, u32 argc, char** argv, int optind) {
  u8* tmp = afl->orig_cmdline;
  u8* aa_loc;
  save_cmdline(afl, argc - optind, argv + optind);

  afl->exec_cmdline = afl->orig_cmdline;
  afl->orig_cmdline = tmp;
  aa_loc = strstr(afl->exec_cmdline, "@@");
  if (aa_loc == 0)
...
void set_fuzzer_id(afl_state_t *afl) {
  ...
  if (env)
    afl->fuzzer_id = ck_strdup(env);
  else {
    const u8* hexdigits = "0123456789abcdef";
    const u8* prefix = "fuzzer-";
    // fuzzer-[8byte hexdigits]
    afl->fuzzer_id =  ck_alloc(strlen(prefix) + 8 + 1);
    strcpy(afl->fuzzer_id, prefix);
    for (int i = strlen(prefix); i < strlen(prefix) + 8; i++)
      afl->fuzzer_id[i] = hexdigits[UR(afl, strlen(hexdigits))];
  }

  // Save fuzzer_id for debugging
  fn = alloc_printf("%s/.fuzzer_id", afl->out_dir);
  fd = open(fn, O_WRONLY | O_CREAT | O_TRUNC, 0600);
  if (fd < 0) PFATAL("Unable to open '%s'", fn);
  ck_write(fd, afl->fuzzer_id, strlen(afl->fuzzer_id), fn);
  close(fd);
  ck_free(fn);
}
/////////////////////////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////////////////////////
  save_exec_cmdline(afl, argc, argv, optind);
/////////////////////////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  if (unlikely(fast_resume)) {
    ...
    if (likely(!afl->afl_env.afl_no_startup_calibration)) {
      ACTF("PERFORM DRY RUN START BUT DISABLED");
      //perform_dry_run(afl);
