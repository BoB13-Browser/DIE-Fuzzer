...
static u32 UR(afl_state_t *afl, u32 limit) {

  if (unlikely(!afl->rand_cnt--)) {

    u32 seed[2];

    ck_read(afl->fsrv.dev_urandom_fd, &seed, sizeof(seed), "/dev/urandom");

    srandom(seed[0]);
    afl->rand_cnt = (RESEED_RNG / 2) + (seed[1] % RESEED_RNG);
...
s32 generate_js(afl_state_t *afl, u8 *own_loc, u8* cur_input, u8* tmp_outdir);
int execute_sh(char* cmd);

void save_exec_cmdline(afl_state_t *afl, u32 argc, char** argv, int optind);
void set_fuzzer_id(afl_state_t *afl);

static u8 fuzz_js(afl_state_t *afl) {
  ...
  fuzz_status = generate_js(afl, afl->own_loc, cur_input, fuzz_inputs_dir);
...
static void globally_sync(afl_state_t *afl, bool crash) {
  u8* cmdline, *fn, *cmd, *bitmap_name, *orig_bitmap;
  u8 trace_bits_tmp[MAP_SIZE];
  int fd;

  if (!crash) {
    orig_bitmap = afl->virgin_bits;
    bitmap_name = ".gcov_path";
    cmd = "pathBitmap";
  }
  else {
    orig_bitmap = afl->virgin_crash;
    bitmap_name = ".gcov_crash";
    cmd = "crashBitmap";
  }

  fn = alloc_printf("%s/%s", afl->out_dir, bitmap_name);
  cmdline = alloc_printf("node %s/../TS/redis_ctrl.js downloadBitmap %s %s",
      afl->own_loc, cmd, fn);

  execute_sh(cmdline);

  memcpy(trace_bits_tmp, afl->fsrv.trace_bits, MAP_SIZE);
  ...
  ck_read(fd, afl->fsrv.trace_bits, MAP_SIZE, fn);
  close(fd);

  has_new_bits(afl, orig_bitmap);

cleanup:
  memcpy(afl->fsrv.trace_bits, trace_bits_tmp, MAP_SIZE);
}
/////////////////////////////////////////////////////////////////////////////////////////////////